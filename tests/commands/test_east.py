...
class TestSuiteEast:
    ...
    def test_when_moves_to_east_then_new_room_must_be_on_east_from_actual(self, cmd, game_context):
        # arrange
    ...
    def test_if_there_is_no_room_on_east_then_current_room_remains_after_going_east(self, cmd, game_context):
        # act
        command = parse_line(cmd.name, game_context.commands)
        command.exec(game_context)
    ...
    def test_if_there_is_no_room_on_east_then_error_message_should_appear(self, cmd, game_context, capsys):
        # act
        command = parse_line(cmd.name, game_context.commands)
        command.exec(game_context)
